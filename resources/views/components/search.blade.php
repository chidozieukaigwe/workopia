<form method="GET" action="{{ route('jobs.search') }}" class="block mx-5 md:mx-auto md:space-x-2">
    <input type="text" name="keywords" placeholder="Keywords"
        class="w-full md:w-72 px-4 py-3 focus:outline-none bg-white rounded" value="{{ request('keywords') }}" />
    <input type="text" name="location" placeholder="Location"
        class="w-full md:w-72 px-4 py-3 focus:outline-none bg-white rounded" value="{{ request('location') }}" />
